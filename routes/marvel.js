...
router.get("/comics", async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `https://lereacteur-marvel-api.herokuapp.com/comics?apiKey=${API_KEY_MARVEL}&title=${title}&skip=${skip}&limit=${limit}`
    );
...
router.get("/characters", async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `https://lereacteur-marvel-api.herokuapp.com/characters?apiKey=${API_KEY_MARVEL}&name=${name}&skip=${skip}&limit=${limit}`
    );
    res.json(response.data);
  } catch (error) {
    res.status(400).json({ message: error.message });
